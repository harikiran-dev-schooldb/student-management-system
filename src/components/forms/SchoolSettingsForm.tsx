...
const SchoolSettingsForm = ({ initialData, userRole }: Props) => {
  ...
  const { register, handleSubmit, reset, watch } = useForm<Partial<SchoolInfo>>({
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="max-w-7xl mx-auto space-y-6 pb-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
          ...
          <div className="flex items-center gap-5">
            ...
            <div>
              ...
              <p className="text-gray-500 dark:text-gray-400 font-medium">
                {isAdmin ? "Configure institutional branding & metadata" : "Official contact and identity details"}
              </p>
            </div>
          </div>
            ...
            <div className="flex items-center gap-3">
              {!isEditing ? (
                ...
                <div className="flex gap-2 animate-in zoom-in-95 duration-200">
                  <button
                    type="button"
                    onClick={() => { setIsEditing(false); reset(initialData || {}); }}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="xl:col-span-2 space-y-6">
          ...
          <Section cardTitle="Identity Details" icon={<FileText size={20} className="text-blue-500" />}>
          ...
          <Section cardTitle="Contact Information" icon={<MapPin size={20} className="text-orange-500" />}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <InputGroup label="Phone Number" name="phone" register={register} disabled={!isEditing} icon={<Phone size={18} />} />
              <InputGroup label="Official Email" name="email" register={register} disabled={!isEditing} icon={<Mail size={18} />} />
              <div className="md:col-span-2">
                <InputGroup label="Website Link" name="website" register={register} disabled={!isEditing} icon={<Globe size={18} />} />
              </div>
              <div className="md:col-span-2">
                <label className="block text-xs font-bold text-gray-400 dark:text-gray-500 mb-2 uppercase tracking-widest">Campus Address</label>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white dark:bg-gray-900 rounded-3xl border border-gray-200 dark:border-gray-800 p-8 flex flex-col items-center justify-center text-center shadow-sm relative group">
            <div className="absolute top-4 right-4">
               {initialData?.website && <a href={initialData.website} target="_blank" className="p-2 text-gray-400 hover:text-indigo-600 transition-colors"><ExternalLink size={16}/></a>}
            </div>
            <h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-6">Live Branding Preview</h3>
            <div className="relative w-40 h-40 rounded-3xl border-2 border-dashed border-gray-200 dark:border-gray-800 flex items-center justify-center overflow-hidden bg-gray-50 dark:bg-gray-950/50 group-hover:border-indigo-300 transition-colors">
              {logoUrl ? (
                <img src={logoUrl} alt="Logo" className="w-full h-full object-contain p-4 transition-transform duration-500 group-hover:scale-110" />
              ) : (
                <ImageIcon size={48} className="text-gray-300" />
              )}
            </div>
            <div className="mt-6">
                <p className="text-sm font-bold text-gray-900 dark:text-white">{watch("name") || "School Name"}</p>
                <p className="text-xs text-gray-500 mt-1">Official Institutional Logo</p>
            </div>
          </div>

          <Section cardTitle="Receipts & Printing" icon={<Receipt size={20} className="text-emerald-500" />}>
            <div className="space-y-5">
              <InputGroup label="Header Text" name="receiptHeader" register={register} disabled={!isEditing} icon={<Building size={18} />} />
              <div>
                <label className="block text-xs font-bold text-gray-400 dark:text-gray-500 mb-2 uppercase tracking-widest">Receipt Footer Note</label>
                <textarea
...
const Section = ({ cardTitle, icon, children }: { cardTitle: string; icon: React.ReactNode; children: React.ReactNode }) => (
  <div className="bg-white dark:bg-gray-900 rounded-3xl border border-gray-200 dark:border-gray-800 shadow-sm overflow-hidden flex flex-col">
    <div className="px-6 py-4 border-b border-gray-50 dark:border-gray-800 flex items-center gap-3 bg-gray-50/30 dark:bg-white/5">
      ...
      <h2 className="font-bold text-gray-800 dark:text-white tracking-tight">{cardTitle}</h2>
    </div>
...
interface InputGroupProps {
  ...
  name: keyof SchoolInfo;
  register: UseFormRegister<Partial<SchoolInfo>>;
  disabled: boolean;
  placeholder?: string;
}

const InputGroup = ({ label, icon, name, register, disabled, placeholder }: InputGroupProps) => (
  <div className="w-full">
    <label className="block text-xs font-bold text-gray-400 dark:text-gray-500 mb-2 uppercase tracking-widest leading-none">{label}</label>
    <div className="relative group">
