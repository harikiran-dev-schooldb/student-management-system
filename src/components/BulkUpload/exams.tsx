  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (res.data.errors && res.data.errors.length > 0) {
        setErrors(
          res.data.errors.map(
            (e: any) => `Row ${e.row}: ${typeof e.error === 'object' ? JSON.stringify(e.error) : e.error}`
          )
        );
      } else {
        setSuccess(true);
      ...
      <header className="px-6 py-8 md:px-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="mt-2 text-zinc-500 dark:text-zinc-400 font-medium">
            Schedule multiple exams at once by uploading a CSV file.
          </p>
        </div>

        <button
          onClick={() => window.open('/sample/exams-bulk-template.csv')}
                ...
                {success && (
                  <div className="p-4 rounded-xl bg-emerald-50 dark:bg-darkMode border border-emerald-200 dark:border-emerald-800/50 flex items-center gap-4 animate-in slide-in-from-top-2">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-emerald-600 dark:text-emerald-400">Your exam schedule has been successfully created.</p>
                    </div>
                  </div>
                )}
              </div>
