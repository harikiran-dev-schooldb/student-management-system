...
export default function BulkFeeStructureUpload() {
  ...
  const handleUpload = async () => {
    setLoading(true);
    try {
      const response = await axios.post(`/api/v1/tenants/${schoolId}/fees/feestructure/bulk-upload`, {
        feeStructures,
      });
      ...
      <header className="px-6 py-8 md:px-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="mt-2 text-zinc-500 dark:text-zinc-400 font-medium">
            Define fee structures across grades in bulk via CSV.
          </p>
        </div>

        <button
          onClick={() => window.open('/sample/feeStructure-bulk-template.csv')}
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-zinc-500 dark:text-zinc-400 flex items-center gap-2">
                      ...
                      {feeStructures.length} Structures Found
                    </p>
                  </div>
                </div>
                ...
                {success && (
                  <div className="p-4 rounded-xl bg-emerald-50 dark:bg-darkMode border border-emerald-200 dark:border-emerald-800/50 flex items-center gap-4 animate-in slide-in-from-top-2">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-emerald-600 dark:text-emerald-400">Your fee structures have been successfully updated.</p>
                    </div>
                  </div>
                )}
              </div>
                      ...
                      {feeStructures.map((row, i) => (
                        <tr key={i} className="group hover:bg-indigo-50/30 dark:hover:bg-indigo-500/5 transition-colors">
                          ...
                          {Object.values(row).map((val, j) => (
                            <td key={j} className="px-6 py-3 font-medium text-zinc-700 dark:text-zinc-300">
                              {val ? (
                                <span>{val}</span>
                              ) : (
                ...
                >
                  {loading ? (
                    ...
                    <>
                      ...
                      Import {feeStructures.length} Items
                    </>
                  )}
                </button>
              </div>
