...
import { CalendarClock, CalendarDays, Megaphone, AlertCircle } from "lucide-react";
...
const TeacherPage = async () => {
  ...
  if (!teacher) return <UnauthorizedReload />;
    ...
    <div className="flex flex-col gap-6 p-6 min-h-screen bg-gray-50/50 dark:bg-[#0d1117]">
      ...
      <div>
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
          <CalendarClock className="text-indigo-600 dark:text-indigo-400" size={28} />
          Teacher Dashboard
        </h1>
        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
          Welcome back, <span className="font-semibold text-gray-700 dark:text-gray-200">{teacher.name}</span>
        </p>
      </div>
        ...
        <div className="w-full xl:w-2/3">
          <div className={`${cardClass} h-full`}>
            <div className="flex items-center justify-between mb-6 border-b border-gray-100 dark:border-gray-800 pb-4">
              <h2 className="text-lg font-bold text-gray-900 dark:text-white">My Schedule</h2>
              {teacher.class && (
            ...
            {teacher.id ? (
              ...
              <div className="flex flex-col items-center justify-center h-64 text-center">
                ...
                <p className="text-gray-500 dark:text-gray-400 font-medium">No schedule available</p>
                <p className="text-xs text-gray-400 mt-1">Contact admin if this is an error.</p>
              </div>
            )}
          </div>
        </div>
